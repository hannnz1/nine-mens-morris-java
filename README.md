...
**和电脑对战（M3）：**首页选择“和电脑下一局”，设置简单/中等/困难、执白/随机/执黑和计时，点击“开始对局”。默认中等、随机、5+3。电脑回合显示“电脑思考中…”，人机对局不提供邀请或提和；“再来一局”自动开始并交换执子。网络中断时点击“恢复对局”重试保存的开局请求，不会重复创建。

AI 使用现有规则引擎上的 alpha-beta 搜索，成三与移除属于同一回合。简单深度1（30%随机），中等深度3，困难迭代加深到8、默认预算800ms。每步按游戏ID和版本生成种子；困难模式受时间预算影响，不承诺跨机器着法一致。

公开棋局操作的 `Idempotency-Key` 不得使用 `bot:` 前缀，该命名空间保留给内部机器人动作，违规返回 `400 INVALID_IDEMPOTENCY_KEY`。

| 人机配置环境变量 | 默认值 |
| --- | --- |
| `BOT_THREADS` | 1 |
| `BOT_MAX_ACTIVE_GAMES` | 20 |
| `BOT_HARD_BUDGET_MS` | 800 |
| `BOT_THINK_DELAY_MIN_MS` / `BOT_THINK_DELAY_MAX_MS` | 400 / 800 |

全站20局上限通过事务行锁控制；每位真人最多5局，电脑不受个人局数限制。无排队时目标2秒内应手；20局同时请求时会排队，不能保证每局均在2秒内应手。每2秒扫描停滞超过3秒的电脑回合，连续3次计算失败后下一次用合法随机动作恢复。`morris.scheduling.enabled=false` 同时关闭真实机器人线程池与定时扫描。

真人创建、加入和再战共用玩家行锁保护个人5局上限；涉及两位真人时按UUID顺序加锁。幂等重放和已入座玩家重新进入不消耗新名额。终局或调度器关闭后的迟到计算异常不会重新创建失败计数。
...
node --test scripts/bot-load-test.test.mjs
mvn -pl game-engine -Pstrength test
node scripts/bot-load-test.mjs --base-url http://localhost:18080 --server-log backend.log
```

普通测试包含简单/中等各1000局、困难20ms预算50局的合法性测试和缩小版棋力回归。`strength` 额外运行中等/困难各对简单100局，双方执色各半，和棋不算胜；阈值分别65%、80%，困难每步预算800ms。完整棋力测试可能需要较长时间。

压测只接受显式本地HTTP地址，创建4个身份共20局困难对局，输出真人动作P99、电脑应手延迟及相对空载增长；增长超过20%时退出码为2。使用 `--logging.level.io.github.hannnz1.morris.backend.service.BotTurnScheduler=DEBUG` 启动后端并传入其日志路径，可同时报告排队与搜索时间。压测需要独立空数据库，不要用于生产库；输出是该机器与该负载的实测结果，不能替代生产硬件验收。
